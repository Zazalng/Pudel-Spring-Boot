...
/**
 * Bootstrap runner that ensures all guilds the bot is in have proper database
 * schemas. Runs on startup after JDA is ready.
 *
 * <p>Global (public) tables are owned by Hibernate ({@code spring.jpa.hibernate.ddl-auto: update}),
 * so nothing needs to be done for them here. Per-guild/per-user schemas are
 * reconciled by {@link SchemaManagementService} directly from Java, which
 * creates any missing tables/columns/indexes and repairs existing schemas on
 * every boot. No external init.sql or SQL migration step is required.
 */
@Component
...
public class SchemaBootstrapRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        for (Guild guild : guilds) {
            ...
            // Ensure the per-guild schema exists (creates/repairs tables, columns, indexes).
            ...
            // Reconcile (create/repair) embedding tables for this guild if pgvector is available.
            try {
                schemaManagementService.ensureGuildEmbeddingTables(guildIdLong);
            } catch (Exception e) {
                logger.debug("Embedding tables not ensured for guild {} ({}): {}",
                        guildName, guildId, e.getMessage());
            }
        }
